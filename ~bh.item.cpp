...
interface D2CommonDataTable {
  dwItemsTypeRecs: number;
  pItemsTypeTxt: D2ItemTypesTxt[];
}

// Assuming p_D2COMMON_sgptDataTable is already defined and accessible
declare const p_D2COMMON_sgptDataTable: D2CommonDataTable;

// Maps to hold the extracted values
const throwableMap: Record<number, number> = {};
const bodyLocMap: Record<number, number> = {};
const parentMap1: Record<number, number> = {};
const parentMap2: Record<number, number> = {};

function getItemTypeMaps(): void {
  const { dwItemsTypeRecs, pItemsTypeTxt } = p_D2COMMON_sgptDataTable;

  for (let d = 0; d < dwItemsTypeRecs; d++) {
    const item = pItemsTypeTxt[d];

    if (item.szCode) {
      throwableMap[d] = item.nThrowable;
      bodyLocMap[d] = item.nBodyLoc1;

      if (item.nEquiv1) {
        parentMap1[d] = item.nEquiv1;
      }

      if (item.nEquiv2) {
        parentMap2[d] = item.nEquiv2;
      }
    }
  }
}
